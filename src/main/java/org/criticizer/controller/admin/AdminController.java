...
public class AdminController {
    ...
    public AdminController(
            UserService userService, DashboardService dashboardService, SecurityUtil securityUtil) {
    ...
    public ResponseEntity<PageResponse<UserAdminResponse>> getUsers(
        ...
        log.debug(
                "Admin requesting users list - page: {}, size: {}, search: {}", page, size, search);

        PageResponse<User> result = userService.getUsersPage(search, page + 1, size, false);

        List<UserAdminResponse> userResponses =
                result.getItems().stream().map(UserAdminResponse::from).toList();

        Page<UserAdminResponse> pageResult =
                new PageImpl<>(userResponses, PageRequest.of(page, size), result.getTotalItems());
    ...
    public ResponseEntity<MessageResponse> changeUserRole(
            @PathVariable int userId, @Valid @RequestBody ChangeRoleRequest request) {
