...
/**
 * Abstract base service for all media entities (Game, Movie, Book, Show). Contains common business
...
public abstract class AbstractMediaService<T extends MediaEntity, R> {
    ...
    public PageResponse<T> getUserItemsPage(
            ...
            String sortBy,
            String sortOrder) {
        return getUserItemsPage(
                userId, page, pageSize, categoryId, searchTerm, sortBy, sortOrder, null, null);
    }

    /** Get paginated items with filtering, search and score range. */
    public PageResponse<T> getUserItemsPage(
            Integer userId,
            Integer page,
            Integer pageSize,
        ...
        Page<Integer> itemIds =
                repository.findItemIds(
                        userId, categoryId, sanitizedSearch, minScore, maxScore, pageable);
        ...
        log.debug(
                "Fetched {} {}s (page {} of {}) for user {}",
                items.size(),
                getEntityName(),
                params.page(),
                itemIds.getTotalPages(),
                userId);

        //  Create PageImpl manually with items and pagination info from itemIds
        Page<T> itemPage =
                new org.springframework.data.domain.PageImpl<>(
                        items, pageable, itemIds.getTotalElements());
    ...
    public void addItem(
        ...
        if (repository.existsByNameIgnoreCaseAndUserId(trimmedName, userId)) {
            log.warn(
                    "Attempted to add duplicate {} '{}' for user {}",
                    getEntityName(),
                    trimmedName,
                    userId);
    ...
    public void updateItem(
        ...
        T entity =
                repository
                        .findByNameIgnoreCaseAndUserId(trimmedOldName, userId)
                        .orElseThrow(
                                () ->
                                        new ResourceNotFoundException(
                                                getEntityName(), trimmedOldName));
        ...
        log.info("Updated {} '{}' to '{}' for user {}", getEntityName(), oldName, newName, userId);
    }
    ...
    public void removeItem(String name, Integer userId) {
        ...
        T entity =
                repository
                        .findByNameIgnoreCaseAndUserId(trimmedName, userId)
                        .orElseThrow(
    ...
    public void updateCover(String name, String coverUrl, Integer userId) {
        ...
        entity.setCoverUrl(coverUrl != null && !coverUrl.trim().isEmpty() ? coverUrl.trim() : null);

        repository.save(entity);

        log.info("Updated cover for {} '{}' (user {})", getEntityName(), name, userId);
    }
    ...
    public PageResponse<R> getUserItemsPageAsDto(
            ...
            String sortBy,
            String sortOrder) {
        return getUserItemsPageAsDto(
                userId, page, pageSize, categoryId, searchTerm, sortBy, sortOrder, null, null);
    }

    /** Returns a paginated page of DTOs with score range filter. */
    public PageResponse<R> getUserItemsPageAsDto(
            Integer userId,
            Integer page,
            Integer pageSize,
        ...
        List<R> dtos = entityPage.getItems().stream().map(this::toResponse).toList();
