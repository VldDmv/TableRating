...
public class ServiceValidator {
    ...
    public String validateUsername(String name) {
        ...
        if (trimmed.length() > MAX_USERNAME_LENGTH) {
            log.warn(
                    "Username validation failed: too long ({} chars, max {})",
                    trimmed.length(),
                    MAX_USERNAME_LENGTH);
    ...
    public void validatePassword(String password) {
        ...
        if (password.length() > MAX_PASSWORD_LENGTH) {
            log.warn(
                    "Password validation failed: password is too long ({} chars, max {})",
                    password.length(),
                    MAX_PASSWORD_LENGTH);
            throw new InvalidInputException(
    ...
    public String sanitizeSearchTerm(String searchTerm) {
        ...
        if (trimmed.length() > MAX_SEARCH_TERM_LENGTH) {
            log.warn(
                    "Search term too long: {} chars (max {})",
                    trimmed.length(),
    ...
    public String validateName(String name, String fieldName) {
        ...
        if (trimmed.length() > 255) {
            log.warn("{} validation failed: too long ({} chars)", fieldName, trimmed.length());
            throw new InvalidInputException(
                    fieldName.toLowerCase(), "must not exceed 255 characters");
        }
