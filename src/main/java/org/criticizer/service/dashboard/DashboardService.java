 ...
 *
 * <p>Performance improvement in getUserDashboardStats(): Before: 5 queries per media type × 4 types
 * = 20 queries per dashboard load. After: 1 query per media type × 4 types = 4 queries per
 * dashboard load.
 *
 * <p>Uses MediaRepository.getStatsByUserId() which returns Object[5]: [count, avgScore, maxScore,
 * minScore, completedCount]
 */
@Service
...
public class DashboardService {
    ...
    /** Returns aggregate counts for the admin dashboard. 5 queries — already optimal. */
    public AdminStats getAdminDashboardStats() {
        ...
        log.debug(
                "Admin stats: {} users, {} games, {} movies, {} books, {} shows",
                totalUsers,
                totalGames,
                totalMovies,
                totalBooks,
    ...
    /**
     * Returns per-media-type stats for a user's dashboard. 4 queries total (one aggregation per
     * media type).
     */
    ...
    /**
     * Single aggregation query → Map with all stats. Object[0] = COUNT, [1] = AVG, [2] = MAX, [3] =
     * MIN, [4] = SUM(completed)
     */
