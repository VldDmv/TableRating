...
public interface BookRepository extends MediaRepository<Book> {
    ...
    @Query(
            "SELECT b.id FROM Book b LEFT JOIN b.genres g WHERE b.userId = :userId AND (:categoryId"
                + " IS NULL OR g.id = :categoryId) AND (:searchTerm IS NULL OR LOWER(b.name) LIKE"
                + " LOWER(CONCAT('%', :searchTerm, '%'))) AND (:minScore IS NULL OR b.score >="
                + " :minScore) AND (:maxScore IS NULL OR b.score <= :maxScore) GROUP BY b.id")
    ...
    @Query("SELECT DISTINCT b FROM Book b " + "LEFT JOIN FETCH b.genres " + "WHERE b.id IN :ids")
    ...
    @EntityGraph(attributePaths = {"genres"})
    @Query(
            "SELECT DISTINCT b FROM Book b WHERE LOWER(b.name) = LOWER(:name) AND b.userId ="
                    + " :userId")
