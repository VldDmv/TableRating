...
public interface MovieRepository extends MediaRepository<Movie> {
    ...
    @Query(
            "SELECT m.id FROM Movie m LEFT JOIN m.genres g WHERE m.userId = :userId AND"
                + " (:categoryId IS NULL OR g.id = :categoryId) AND (:searchTerm IS NULL OR"
                + " LOWER(m.name) LIKE LOWER(CONCAT('%', :searchTerm, '%'))) AND (:minScore IS NULL"
                + " OR m.score >= :minScore) AND (:maxScore IS NULL OR m.score <= :maxScore) GROUP"
                + " BY m.id")
    ...
    @EntityGraph(attributePaths = {"genres"})
    @Query(
            "SELECT DISTINCT m FROM Movie m WHERE LOWER(m.name) = LOWER(:name) AND m.userId ="
                    + " :userId")
