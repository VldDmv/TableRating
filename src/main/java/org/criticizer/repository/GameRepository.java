...
public interface GameRepository extends MediaRepository<Game> {
    ...
    @Query(
            "SELECT g.id FROM Game g LEFT JOIN g.tags t WHERE g.userId = :userId AND (:categoryId"
                + " IS NULL OR t.id = :categoryId) AND (:searchTerm IS NULL OR LOWER(g.name) LIKE"
                + " LOWER(CONCAT('%', :searchTerm, '%'))) AND (:minScore IS NULL OR g.score >="
                + " :minScore) AND (:maxScore IS NULL OR g.score <= :maxScore) GROUP BY g.id")
    ...
    @Query("SELECT DISTINCT g FROM Game g " + "LEFT JOIN FETCH g.tags " + "WHERE g.id IN :ids")
    ...
    @EntityGraph(attributePaths = {"tags"})
    @Query(
            "SELECT DISTINCT g FROM Game g WHERE LOWER(g.name) = LOWER(:name) AND g.userId ="
                    + " :userId")
