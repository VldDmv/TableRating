 ...
 * @deprecated Use PaginationManager class instead.
 */
export function initPagination(config) {
    ...
    const manager = new PaginationManager(elements);

    if (config.onPageChange) {
        manager.onPageChange(config.onPageChange);
    }
    if (config.onRowsPerPageChange) {
        manager.onRowsPerPageChange(config.onRowsPerPageChange);
    }

    manager.init();

    // Setup callback for updates
    if (config.setUpdateCallback) {
        config.setUpdateCallback((pageResult) => manager.update(pageResult));
    }
