...
class GameServiceTest {
    ...
    class GetUserItemsPageTests {
        ...
        void shouldReturnPaginatedGames() {
            ...
            when(gameRepository.findItemIds(eq(TEST_USER_ID), isNull(), isNull(), any(), any(), any(), any()))
                    .thenReturn(gameIds);
        ...
        void shouldFilterBySearchTerm() {
            ...
            when(gameRepository.findItemIds(
                    eq(TEST_USER_ID), isNull(), eq("witcher"), any(), any(), any(), any()))
                    .thenReturn(gameIds);
        ...
        void shouldFilterByTag() {
            ...
            // Then
            assertThat(result.getItems()).hasSize(1);
            verify(gameRepository).findItemIds(eq(TEST_USER_ID), eq(1), isNull(), any(), any(), any(), any());
        }
    }
