...
class E2EIntegrationTest {
    ...
    void completeUserJourney_RegisterLoginCreateGameLogout() throws Exception {
        ...
        // 6. Delete game
        mockMvc.perform(
                        delete("/api/games/Dark Souls Remastered")
                                .with(user(auth(newUser)))
                                .with(csrf()))
                .andExpect(status().isOk())
    ...
    void adminWorkflow_ManageUsersAndViewStats() throws Exception {
        ...
        String roleJson = """
                ...
                """;
        mockMvc.perform(
                        put("/api/admin/users/" + testUser.getId() + "/role")
        ...
        mockMvc.perform(get("/api/admin/users/" + testUser.getId()).with(user(auth(adminUser))))
                .andExpect(status().isOk())
    ...
    void tagManagementWorkflow() throws Exception {
        ...
        // 6. Delete tag (ADMIN)
        mockMvc.perform(delete("/api/tags/" + tagId).with(user(auth(adminUser))).with(csrf()))
                .andExpect(status().isOk())
    ...
    void paginationWorkflow() throws Exception {
        for (int i = 1; i <= 15; i++) {
            String gameJson =
                    String.format("{\"name\":\"Game %d\",\"score\":%d,\"tagIds\":[]}", i, 50 + i);
            mockMvc.perform(
                            post("/api/games")
                                    .with(user(auth(testUser)))
                                    .contentType(MediaType.APPLICATION_JSON)
